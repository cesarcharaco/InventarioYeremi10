@extends('layouts.app')

@section('title') Clientes @endsection

@section('content')
<main class="app-content">
  <div class="app-title">
    <div>
      <h1><i class="fa fa-users"></i> Clientes</h1>
      <p>Gestión de Cartera de Clientes | Yermotos Repuestos C.A.</p>
    </div>
    <ul class="app-breadcrumb breadcrumb">
      <li class="breadcrumb-item"><a href="{{ route('home') }}"><i class="fa fa-home fa-lg"></i></a></li>
      <li class="breadcrumb-item"><a href="{{ route('clientes.index') }}">Clientes</a></li>
      <li class="breadcrumb-item"><a href="#">Listado</a></li>
    </ul>
  </div>

  <div class="tile mb-4">
    <div class="row">
      <div class="col-lg-12">
        <div class="page-header">
          <h2 class="mb-3 line-head" id="indicators">Clientes Registrados
            {{-- PERMISO: SuperAdmin, Encargado y Vendedor pueden registrar --}}
            @can('gestionar-clientes')
            <a class="btn btn-primary icon-btn pull-right" href="{{ route('clientes.create') }}">
              <i class="fa fa-plus"></i> Registrar Cliente
            </a>
            @endcan
          </h2>
        </div>
        <div class="basic-tb-hd text-center">            
          @include('layouts.partials.flash-messages')
        </div>
      </div>
    </div>

    <div class="row">
      <div class="col-md-12">
        <div class="tile">
          <div class="tile-body">
            <div class="table-responsive">
              <table class="table table-hover table-bordered" id="sampleTable">
                <thead>
                  <tr>
                    <th>Identificación</th>
                    <th>Nombre y Apellido</th>
                    <th>Teléfono</th>
                    <th>Sede Registro</th>
                    <th>Límite Crédito</th>
                    <th>Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  @foreach($clientes as $cliente)
                  <tr>
                    <td><span class="badge badge-dark">{{ $cliente->identificacion }}</span></td>
                    <td>{{ $cliente->nombre }}</td>
                    <td>{{ $cliente->telefono }}</td>
                    <td>{{ $cliente->local->nombre ?? 'N/A' }}</td>
                    <td>
                        <strong class="text-success">{{ number_format($cliente->limite_credito, 2) }}$</strong>
                    </td>
                    <td>
                      {{-- Botón VER Detalle --}}
                      <a href="{{ route('clientes.show', $cliente->id) }}" 
                         class="btn btn-info btn-sm" 
                         data-toggle="tooltip" title="Ver Detalles">
                        <i class="fa fa-eye"></i>
                      </a>

                      {{-- PERMISO: Editar --}}
                      @can('gestionar-clientes')
                      <a href="{{ route('clientes.edit', $cliente->id) }}" 
                         class="btn btn-primary btn-sm" 
                         data-toggle="tooltip" title="Editar Cliente">
                        <i class="fa fa-edit"></i>
                      </a>
                      @endcan

                      {{-- PERMISO: Eliminar (Solo SuperAdmin) --}}
                      @can('eliminar-clientes')
                      <a href="javascript:;" 
                         class="btn btn-danger btn-sm" 
                         data-toggle="modal" 
                         data-target="#eliminar_Cliente" 
                         onclick="eliminar('{{ $cliente->id }}', '{{ $cliente->nombre }}')">
                        <i class="fa fa-trash"></i>
                      </a>
                      @endcan

                      @cannot('gestionar-clientes')
                        <span class="badge badge-light">Lectura</span>
                      @endcannot
                    </td>
                  </tr>
                  @endforeach
                </tbody>
              </table>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</main>

{{-- Modal Eliminar --}}
<div class="bs-component">
  <div class="modal" id="eliminar_Cliente">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title text-danger"><i class="fa fa-trash"></i> Eliminar Cliente</h5>
          <button class="close" type="button" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span>
          </button>
        </div>
        {{-- Usamos el mismo estilo de Form Collective que tienes en Categorías --}}
        {!! Form::open(['route' => ['clientes.destroy', 0], 'method' => 'DELETE', 'id' => 'form-eliminar-cliente']) !!}
          <div class="modal-body text-center">
              <h2 class="text-warning"><i class="fa fa-exclamation-triangle"></i></h2>
              <h4>¿Está seguro que desea eliminar al cliente?</h4>
              <p id="nombre_cliente_modal" class="font-weight-bold"></p>
              <p class="text-muted small">Esta acción no se podrá deshacer y podría afectar el historial de créditos.</p>
              <input type="hidden" name="id_cliente" id="id_cliente_input">
          </div>
          <div class="modal-footer">
              <button class="btn btn-danger" type="submit">Confirmar Eliminación</button>
              <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancelar</button>
          </div>          
        {!! Form::close() !!}
      </div>
    </div>
  </div>
</div>
@endsection

@section('scripts')
<script type="text/javascript">
  // Función para preparar el modal de eliminación
  function eliminar(id, nombre) {
    $("#id_cliente_input").val(id);
    $("#nombre_cliente_modal").text(nombre);
    $('#form-eliminar-cliente').attr('action', "{{ url('clientes') }}/" + id);
  }

  $(document).ready(function() {
    // Inicialización de DataTable con idioma español
    if ( ! $.fn.DataTable.isDataTable( '#sampleTable' ) ) {
        $('#sampleTable').DataTable({
          "language": { 
              "url": "//cdn.datatables.net/plug-ins/1.13.6/i18n/es-ES.json" 
          },
          "responsive": true,
          "autoWidth": false,
          "order": [[ 1, "asc" ]] // Ordenar por nombre por defecto
        });
    }

    // Inicializar tooltips
    $('[data-toggle="tooltip"]').tooltip();
  });
</script>
@endsection